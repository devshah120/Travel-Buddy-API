...
router.post('/', user_jwt, async (req, res, next) => {
    try {
       const Bus1 = await Bus.create({ name: req.body.name, email: req.body.email, from: req.body.from, to: req.body.to,user: req.user.id,people:req.body.people,date:req.body.date,time:req.body.time,classs:req.body.classs});
       if(!Bus1) {
